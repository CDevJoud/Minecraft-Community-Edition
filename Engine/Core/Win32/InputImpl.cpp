...
sf::priv::EnumArray<sf::Keyboard::Scancode, sf::Keyboard::Key, sf::Keyboard::Scancode::ScancodeCount> scancodeToKeyMapping; ///< Mapping from Scancode to Key
...
void ensureMappings()
{
    ...
    for (unsigned int i = 0; i < sf::Keyboard::Scancode::ScancodeCount; ++i)
    {
...
{
    return code > sf::Keyboard::Scan::Unknown && static_cast<unsigned int>(code) < sf::Keyboard::Scancode::ScancodeCount;
}
...
{
    if (const WindowHandle handle = relativeTo.getSystemHandle())
    {
        POINT point;
        GetCursorPos(&point);
