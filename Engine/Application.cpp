...
namespace dms {
	...
	Application::Application(int argc, char* argv[]) :
		...
		/*console({
			...
			{"window", {
					...
					{"title", "Profile Community Edition Debugger"},
	...
	void Application::setupLogging() {
		...
		//eastl::shared_ptr<dms::io::FileSink> fileSink = eastl::make_shared<dms::io::FileSink>(getLogFileName());

		////dms::io::Logger& logger = dms::io::Logger::getGlobalLogger(qBus);
	...
	void Application::createProfile(const eastl::string profileName, eastl::unique_ptr<sf::WindowBase> window) {
		...
		eastl::unique_ptr<Profile> mc = eastl::make_unique<Profile>(
			profileName,
			Application::qBus,
			viewId,
		...
		Profile* rawMinecraftPtr = mc.get();
		sf::WindowBase* rawWindowPtr = window.get();
		Application::instances.emplace_back(
			eastl::make_pair<eastl::unique_ptr<sf::WindowBase>, eastl::unique_ptr<Profile>>(eastl::move(window), eastl::move(mc))
		);
		...
		threadManager.createThread(profileName, [this, rawMinecraftPtr]() {
			...
			auto it = eastl::find_if(instances.begin(), instances.end(), [rawMinecraftPtr](const eastl::pair<eastl::unique_ptr<sf::WindowBase>, eastl::unique_ptr<Profile>>& m) {
