...
namespace dms {
	...
	Profile::Profile(
		...
		rmlContextName = Profile::profileName + "_RmlUI:main";
		...
		Profile::vb = factory.createVertexBuffer(vArray, vbFlag, "Profile:VertexBuffer");
		if (Profile::vb == nullptr) {
	...
	int Profile::run() {
		...
		int retCode = Profile::initInstance();

		if (retCode < 0) {
			return retCode;
		}

		while (Profile::bIsRunning) {
	...
	void Profile::render() {
		...
		//renderer.render(Profile::vb, renderState);
